...
```bash
cd app && npm run publicar -- --mensaje "Qué cambia"   # build + pa app push + git commit + git push
...
| **Proyectos** | Cartera de proyectos de cliente e internos, con filtros por estado y tipo. Cada proyecto se organiza en **apartados** (tecnología, fase o entregable), cada uno con su avance, sus tareas y alta rápida escribiendo en él; al crear un proyecto se puede partir de una plantilla de apartados («por tecnología» o «por fases»). La ficha enlaza la carpeta de SharePoint del proyecto. Los apartados se guardan en `loc_proyecto.loc_apartados` (JSON) y cada tarea sabe el suyo en `loc_tarea.loc_apartado`; una tarea cambia de apartado desde su detalle. |
